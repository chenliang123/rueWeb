...
<link href="<?php echo (MANAGE); ?>static/h-ui/css/countReport.css?18" rel="stylesheet" type="text/css"/>
<!--[if lt IE 9]>
...
<script type="text/javascript" src="<?php echo (MANAGE); ?>static/h-ui/js/report.js?is=150"></script>
	...
	function getReportData(dayfrom,dayto,days){
		...
		if(page == '0'){
			...
			$data.getReport1(dayfrom,dayto,days,schoolid,function(out){
		    	...
		    	html = "<div class='panel-header'>教师如e课时和课堂积分累计表</div><div class='panel-body'><div class='chart-box' id='chart' style='width: 960px;height: 450px;margin-top: 34px;'></div><div class='tableData'><table class='table table-border table-striped'><thead><tr><th>姓名</th><th>年级</th><th>科目</th><th>课表课时数</th><th>如e课时数</th><th>如e课时数比例</th><th>课堂积分</th><th>课均积分</th></tr></thead><tbody>";
		    	var tempIndex = 0;
...
//		    	});
	   		...
	   		$data.getReport2(dayfrom,dayto,days,schoolid,function(out){
		    	...
		    	var photoArr = [];
		    	var pizhuArr = [];
		    	html = "<div class='panel-header'>教师课堂教学行为统计表</div><div class='panel-body'><div class='chart-box' id='chart' style='width: 960px;height: 450px;margin-top: 34px;'></div><div class='tableData'><table class='table table-border table-striped'><thead><tr><th>教师姓名</th><th>年级</th><th>科目</th><th>如e课时数</th><th>讲授课件数<br/>(平均每课时)</th><th>互动次数<br/>(平均每课时)</th><th>提问次数<br/>(平均每课时)</th><th>点名次数<br/>(平均每课时)</th><th>出题次数<br/>(平均每课时)</th><th>奖励次数<br/>(平均每课时)</th><th>拍照次数<br/>(平均每课时)</th><th>批注次数<br/>(平均每课时)</th></tr></thead><tbody>";
...
//		    	console.log(dataArr)
		    	...
		    	html = "<div class='panel-header'>教师课堂教学动作时长比例表</div><div class='panel-body'><div class='chart-box' id='chart' style='width: 960px;height: 450px;margin-top: 34px;'></div><div class='tableData'><table class='table table-border table-striped'><thead><tr><th>教师姓名</th><th>年级</th><th>科目</th><th>如e课时数</th><th>讲授课件时长比例<br/>(平均每课时)</th><th>提问时长比例<br/>(平均每课时)</th><th>出题时长比例<br/>(平均每课时)</th><th>其他如e时长比例<br/>(平均每课时)</th><th>非如e时长比例<br/>(平均每课时)</th></tr></thead><tbody>";
				var nameData = [];
				var teachArr = [];
				var askArr = [];
				var xitiArr = [];
...
//		    	console.log(out)
	   		...
	   		$data.getReport5('20170500','20170531','32',schoolid,function(out){
		    	...
		    	html = "<div class='panel-header'>班级学生互动表现统计表</div><div class='panel-body'><div class='chart-box' id='chart' style='width: 960px;height: 450px;margin-top: 34px;'></div><div class='tableData'><table class='table table-border table-striped'><thead><tr><th>班级</th><th>学生数</th><th>课表课时数</th><th>如e课时数</th><th>课均互动次数</th><th>课均互动参与度</th><th>课均不活跃学生数</th><th>课均参与互动均衡度</th></tr></thead><tbody>";
		    	for(var i=0;i < dataArr.length;i++){
		    		...
		    		nameArr.push(dataArr[i].classname);
		    		takepartArr.push(dataArr[i].ave_takePart);
		    		balanceArr.push(dataArr[i].ave_balance);
		    	}
		    	console.log(nameArr)
		    	html += "</tbody></table></div></div>";
		    	$('.countInfo').html(html);
				indexChart5(nameArr,takepartArr,balanceArr);
		    	layer.msg('数据请求成功');
		    	console.log(out)
		    });
	   }else if(page == '5'){
...
//		    	console.log(dataArr)
		    	...
		    	html = "<div class='panel-header'>年级和科目学生互动表现统计表</div><div class='panel-body'><div class='chart-box' id='chart' style='width: 960px;height: 450px;margin-top: 34px;'></div><div class='tableData'><table class='table table-border table-striped'><thead><tr><th>年级</th><th>科目</th><th>课表课时数</th><th>如e课时数</th><th>课均互动次数</th><th>课均互动参与度</th><th>课均不活跃学生数</th><th>课均参与互动均衡度</th></tr></thead><tbody>";
		    	for(var i=0;i < dataArr.length;i++){
		    		...
		    		nameArr.push(dataArr[i].gradeName+'\n'+dataArr[i].coursename);
		    		takepartArr.push(dataArr[i].ave_takePart);
		    		balanceArr.push(dataArr[i].ave_balance);
		    	}
		    	html += "</tbody></table></div></div>";
		    	$('.countInfo').html(html);
				indexChart6(nameArr,takepartArr,balanceArr);
		    	layer.msg('数据请求成功');
		    	console.log(out)
		    });
	   }
